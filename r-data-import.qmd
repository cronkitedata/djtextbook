...
4. [Copy and paste the top section from the Quarto lesson](r-quart.qmd#frontmatter) delete everything else. Try rendering it to check for errors before you go any further -- this part is really picky. 

5.  Copy and paste the code chunk below  into your document and press the "Play" button to run the code.

::: 

[This assumes you  have installed the packages from the [Starting R](r-start.qmd#installing-packages-for-this-course) chapter. Go back and do that now if you skipped it. That step has to be done only once on each computer you use.]{.small}
